/*
	comp2432 project G16
	---------------------------------------------------------------
	Fok Man Him 19080433D
	Yeung Chun Lok 19080113D
	Cheuk Lok Kan 19053031D
	Chan Kwan Wah 19049893D
	---------------------------------------------------------------
...
void printByTenants(struct Event* eventList, char *algorithm, int total_events) {
	...
	for (t = 0; t < 5; t++) {            			
		...
		for(j = 0; j < MAX_EVENT; j++) {
			if (strcmp(eventList[j].tenant,tenants[t]) == 0) {
				tenant_events[t]++;
				// print the event
...
void printSummary(int* time_fcfs, int* time_prio, int totalEvents, int invalidRequest) {
	...
	*/
	...
	fprintf(fp,"          Number of Bookings Assigned: %f (%.2f)\n",accepted_fcfs,accepted_fcfs*100/totalEvents);
	fprintf(fp,"          Number of Bookings Rejected: %f (%.2f)\n",totalEvents-accepted_fcfs, (totalEvents-accepted_fcfs)*100/totalEvents);
	fprintf(fp,"\n    Utilization of Time Slot:\n");
	fprintf(fp,"        Room_A: -%.2f%%\n",time_fcfs[0]/BASE);
	fprintf(fp,"        Room_B: -%.2f%%\n",time_fcfs[1]/BASE);
	fprintf(fp,"        Room_C: -%.2f%%\n",time_fcfs[2]/BASE);
	fprintf(fp,"        webcam_FHD: -%.2f%%\n",(time_fcfs[3]/BASE + time_fcfs[4]/BASE)/2);	// 2 device, so add both and get avg
	fprintf(fp,"        webcam_UHD: -%.2f%%\n",time_fcfs[5]/BASE);
	fprintf(fp,"        monitor_50: -%.2f%%\n",(time_fcfs[6]/BASE + time_fcfs[7]/BASE)/2);	// 2 device, so add both and get avg
	fprintf(fp,"        monitor_75: -%.2f%%\n",time_fcfs[8]/BASE);
	fprintf(fp,"        projector_2k: -%.2f%%\n",(time_fcfs[9]/BASE + time_fcfs[10]/BASE)/2);	// 2 device, so add both and get avg
	fprintf(fp,"        projector_4k: -%.2f%%\n",time_fcfs[11]/BASE);
	fprintf(fp,"        screen_100: -%.2f%%\n",(time_fcfs[12]/BASE + time_fcfs[13]/BASE)/2);	// 2 device, so add both and get avg
...
int main() {
	...
	*/
    ...
    while(1) {
		...
		if (strncmp(inputCommand_ptr[0],"add",3) == 0 && strcmp(inputCommand_ptr[0],"addBatch") != 0) {
